...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onDisappear {
            stopPeriodicRefresh()
            ContentView.sharedSpeechSynthesizer.stopSpeaking(at: .immediate)
    ...
    func loadLatestNews() {
        print("🔄 Starting to load latest news...")
        isLoading = true
            ...
            if success {
                ...
                self.fetchFromSinaAPI(pageSize: 10) { newsItems in
                    DispatchQueue.main.async {
                        print("📥 Received \(newsItems.count) news items from API")
                        withAnimation(.none) {
                            self.news = newsItems
                        }
                        ...
                        print("✅ Initial load: \(newsItems.count) items, currentPage: \(self.currentPage), hasMore: \(self.hasMore)")
                        print("✅ Marked \(keywordMatching.count) keyword-matching items as seen")
                        print("🔍 News items: \(newsItems.map { $0.text.prefix(50) })")
                    }
                }
            } else {
                DispatchQueue.main.async {
                    print("❌ Basic network test failed")
                    self.isLoading = false
                }
            }
        }
    ...
    func loadMoreNews() {
        guard !isLoadingMore && hasMore else { 
            print("⚠️ Load more blocked: isLoadingMore=\(isLoadingMore), hasMore=\(hasMore)")
            return 
        }
        
        isLoadingMore = true
        let nextPage = currentPage + 1
        print("🔄 Loading page \(nextPage) with \(perPage) items...")
        ...
        fetchFromSinaAPI(pageSize: 10) { newsItems in
            DispatchQueue.main.async {
                ...
                if !newImportantNews.isEmpty {
                    ...
                    // TTS after sound - only speak the first important news
                ...
                if !keywordMatchingNews.isEmpty {
                    ...
                    // TTS for keyword news - only speak the first one
    ...
    private func speakChineseText(_ text: String) {
        guard !isTTSInProgress else {
            print("⚠️ TTS already in progress, skipping duplicate announcement")
            return
        }
        
        let newsTitle = extractTitle(from: text)
        
        guard !newsTitle.isEmpty else { return }
        ...
        // Reset flag after speech completes
    ...
    private func speakChineseFullContent(_ text: String) {
        guard !isTTSInProgress else {
            print("⚠️ TTS already in progress, skipping duplicate keyword announcement")
            return
        }
        ...
        // Reset flag after speech completes (longer delay for full content)
    ...
    private func extractTitle(from text: String) -> String {
        ...
        let words = contentWithTitle.components(separatedBy: " ")
